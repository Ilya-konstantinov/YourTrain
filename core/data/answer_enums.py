...
Базовый функционал: 
- Сделать запрос на расписание от станции(ый) до станции(ый) (/help req)\
- Сохранить определённый запрос, чтобы быстро использовать его из меню (/hel cache_req)\
- Сохранить определённый маршрут, чтобы получать уведомления о нём (/help cache_path)\
- Написать отзыв, который поступит напрямую администратору (/help comment)\
- Сохранить/удалить информацию, которую бот сохранил о пользователе (/help json , /help del)
Так же вы можете использовать команду /educ all чтобы получить более обширное обучение \
или /educ если хотите повторить это обучение.
"""
    WHOLE = """\
...
- Сохранить определённый запрос, чтобы быстро использовать его из меню (/hel cache_req), \
а так же возможность изменить уже сохранённый путь (/help recache_req) \
- Сохранить определённый маршрут, чтобы получать уведомления о нём (/help cache_path), \
а так же возможность изменить уже сохранённый маршрут (/help recache_path)\
