...
async def num_path(user_id: int, args: str) -> str:
    ...
    user_cache_path.pop(user_id)
    if not DBCachePath.cache_path_create(user_id, path):
...
def refresh_whole_path(bot):
    aioschedule.clear()
    paths = DBCachePath.get_whole_cache_path()
    for path in paths:
        dep_time = get_whole_path(path.path_id)[path.dep_st.id][0]
        params = {
            'bot': bot,
            'uid': path['user_id'],
            'pid': path['path_id']
...
async def refresh_path(bot, uid: int, pid: int):
    path_old = DBCachePath.get_one_cache_path(uid, pid)
    ...
    dep_st_f = DBStation.station_by_id(path_old['dep_st'])
